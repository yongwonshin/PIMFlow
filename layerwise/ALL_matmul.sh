...
for (( i = 10 ; i < 101 ; i = i + 10 )); do
  ...
  python3 run_matmul --stat --gpgpusim_config=$GPU --model=$MODEL --split_ratio=$i --kernel_launch_latency=$KERNEL_LAUNCH_LATENCY --n_channel=$N_CHANNEL --n_gwrite=1
  python3 run_matmul --stat --gpgpusim_config=$GPU --model=$MODEL --split_ratio=$i --kernel_launch_latency=$KERNEL_LAUNCH_LATENCY --n_channel=$N_CHANNEL --n_gwrite=2
  python3 run_matmul --stat --gpgpusim_config=$GPU --model=$MODEL --split_ratio=$i --kernel_launch_latency=$KERNEL_LAUNCH_LATENCY --n_channel=$N_CHANNEL --n_gwrite=4
  if [ $i -eq 100 ]; then
    ...
    python3 run_matmul --pim --gpgpusim_config=$GPU --model=$MODEL --n_channel=$N_CHANNEL --n_gwrite=4
  fi
