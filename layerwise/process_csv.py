...
def process(model):
    ...
    if args.n_channel == 32:
        newton = pd.read_csv(f'{model}_{TERM}100_32.csv', delimiter=',')
        NEWTON = [list(row) for row in newton.values]
    else:
        newton = pd.read_csv(f'{model}_{TERM}0_{args.n_channel}.csv', delimiter=',')
        NEWTON = [list(row) for row in newton.values]

    Top = pd.read_csv(f'{model}_{TERM}100_{args.n_channel}.csv', delimiter=',')
    ...
    if args.n_channel < 32:
        for i in range(0,91,10):
            print(i)
            f = pd.read_csv(f'{model}_{TERM}{i}_{args.n_channel}.csv',delimiter=',')
    ...
    with open(f'max_performance_{model}_{args.n_channel}.csv', 'w',newline='') as f:
        write = csv.writer(f)
        write.writerow(head)
        write.writerows(MAX)

    with open(f'newton_performance_{model}_{args.n_channel}.csv', 'w',newline='') as f:
        write = csv.writer(f)
